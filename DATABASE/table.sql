...
ALTER TABLE THONGTINGARAGE
ADD PriceIncreasePercent FLOAT NOT NULL DEFAULT 0;

ALTER TABLE THONGTINGARAGE
ADD VatPercent FLOAT NOT NULL DEFAULT 0;

ALTER TABLE TIEPNHANXE
ADD TrangThai NVARCHAR(20) DEFAULT N'DA_TIEP_NHAN';

ALTER TABLE PHIEUTHUTIEN
ADD VatPercent FLOAT DEFAULT 0;

ALTER TABLE PHIEUTHUTIEN
ADD PriceIncreasePercent FLOAT DEFAULT 0;

ALTER TABLE TIEPNHANXE
DROP CONSTRAINT UQ__TIEPNHAN__A780599240D239AE;




UPDATE TIEPNHANXE
SET TrangThai = N'DA_TIEP_NHAN'
WHERE TrangThai IS NULL;


UPDATE T
SET TrangThai = 'DANG_SUA'
FROM TIEPNHANXE T
WHERE EXISTS
(
    SELECT 1
    FROM SUACHUAXE S
    WHERE S.Ma_TiepNhanXe = T.MaTiepNhanXe
);
...
(
    SELECT 1
    FROM PHIEUTHUTIEN P
    WHERE P.Ma_TiepNhanXe = T.MaTiepNhanXe
);
