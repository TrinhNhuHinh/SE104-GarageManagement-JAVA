...
public class ChiTietSuaChuaXeDAO {
    ...
    public boolean insert(ChiTietSuaChuaXe ct) {
        String sql = "INSERT INTO CHITIETSUACHUAXE VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    // PK đơn → 1 tham số (giống HieuXeDAO)
    ...
    // Bonus: lấy tất cả chi tiết của 1 phiếu sửa chữa
    ...
    public boolean update(ChiTietSuaChuaXe ct) {
        String sql = "UPDATE CHITIETSUACHUAXE "
                   + "SET NoiDung = ?, Ma_VatTuPhuTung = ?, SoLuong = ?, DonGia = ?, "
                   + "    Ma_TienCong = ?, ThanhTien = ?, SoTienCong = ? "
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            ps.setString(8, ct.getMaChiTietSuaChuaXe());      // PK đơn ở cuối
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
        }
